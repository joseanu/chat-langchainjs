...
import { BaseDocumentCompressor } from "./base";
import { prompt_template } from "./chain_extract_prompt";

function defaultGetInput(query: string, doc: Document): Record<string, unknown> {
...
export class LLMChainExtractor extends BaseDocumentCompressor {
  ...
  getInput: (query: string, doc: Document) => Record<string, unknown> = defaultGetInput;
